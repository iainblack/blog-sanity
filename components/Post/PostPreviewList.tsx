...
export default function PostPreviewList({ posts }: PostPreviewListProps) {
    ...
    return (
        <div className="flex flex-col items-center w-full">
            {posts?.map((post) => (
                <div key={post._id} className={`w-full sm:w-2/3 md:w-3/4 xl:w-1/2 border border-gray-400 bg-white px-3 py-3 my-2 shadow-md hover:shadow-lg rounded-xl transition-shadow cursor-pointer`}>
                    <Link href={`${path}/posts/${post.slug}`}>
                        <div className="flex flex-col justify-center md:flex-row md:justify-between items-center">
                            <div className="flex flex-col align-center md:flex-row items-center">
